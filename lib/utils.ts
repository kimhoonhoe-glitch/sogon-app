...
export function safeLog(label: string, text: string, maxLength: number = 10): void {
  // 사용자 데이터 로그 최소화 - 처음 10자만
  const preview = text.slice(0, maxLength) + (text.length > maxLength ? '...' : '')
