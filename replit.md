...
## 최근 변경사항
### 2025-10-22: STT 안정성 강화 및 AI 응답 안전성 개선
- **STT 자동 재시작 버그 수정**: 사용자가 마이크를 끄려고 해도 자동 재시작되던 문제 해결
  * app/chat/page.tsx: stopRecording 함수에서 recognition.cleanup() 호출 추가
  * 수동 중지 시 자동 재시작 플래그를 제대로 비활성화
  * UI 상태와 STT 상태 동기화 유지
  * 메모리 누수 방지 (타이머 정리)
- **AI 응답 안전성 강화**: 스트리밍 품질 유지하며 콘텐츠 필터링
  * lib/openai.ts: 전체 응답 수집 → guardrails 필터링 → 5글자씩 20ms 간격으로 재전송
  * 부적절한 내용(오프라인 만남/금전/신체접촉)을 사용자에게 표시하기 전에 차단
  * 스트리밍 효과 유지 (약간의 지연만 발생)
  * 안전성 우선, 사용자 경험도 보존
- **모니터링 포인트**:
  * 5글자/20ms 재전송 타이밍에 대한 사용자 피드백 수집 필요
  * Guardrails가 과도하게 짧은 응답을 생성하는 경우 예외 처리

### 2025-10-22: 페르소나 개선 - 애인 모드 복구 및 동생 모드 자연스럽게 수정
- **애인 모드 복구**: '다정한 톤'을 '애인'으로 되돌림
  * lib/personas.ts: name '다정한 톤' → '애인' 변경
  * description: '사랑스럽고 따뜻한 위로'로 수정
  * greeting: '오늘 무슨 일 있었어?' (반말로 변경)
  * 사랑스럽고 애정 어린 말투 복구
- **동생 모드 자연스럽게 개선**: 인위적인 "오빠/언니" 호칭 제거
  * greeting: '오빠/언니! 오늘 어땠어?' → '오늘 어땠어?'로 변경
  * systemPrompt에서 "오빠/언니" 호칭 강제 사용 제거
  * 응답 예시: 자연스러운 반말로 수정 ("너", "네가" 등)
  * 밝고 친근한 동생의 느낌은 유지하되 더 자연스럽게

### 2025-10-22: 로그인/회원가입 분리 및 페이지 로딩 속도 개선
- **로그인/회원가입 버튼 분리**: 사용자 경험 개선
  * app/page.tsx: 하나의 버튼을 "로그인"과 "회원가입" 두 개로 분리
  * lib/auth.ts: mode 파라미터 추가 ('login' vs 'signup')
  * 로그인: 기존 계정만 인증, 없으면 에러 표시
  * 회원가입: 새 계정만 생성, 이미 있으면 에러 표시
  * 명확한 에러 메시지로 사용자 안내
- **페이지 전환 속도 개선**: 느린 로딩 최적화
  * app/profile/page.tsx: 세션 로딩 중에도 헤더/레이아웃 즉시 표시
  * app/dashboard/page.tsx: 데이터만 로딩 스피너, 전체 페이지는 바로 표시
  * 빈 화면 대기 시간 제거로 체감 속도 향상
  * 레이아웃 shift 없이 부드러운 전환
- **프로필 버튼 추가**: 모든 페이지에서 쉽게 로그아웃 접근
  * 채팅, 대시보드, 프리미엄 페이지 헤더에 👤 프로필 버튼 추가
  * 프로필 페이지에서 빨간색 "로그아웃" 버튼으로 로그아웃

### 2025-10-22: UX 개선 - 로그인 화면 유지 및 로고 버튼 클릭 영역 확대
- **로그인 화면 자동 이동 완전 제거**: 모든 자동 리다이렉트 제거
  * app/page.tsx: useEffect 자동 이동 로직 제거 - 버튼 클릭 시에만 이동
  * app/welcome/page.tsx: welcome_completed 체크 자동 리다이렉트 제거
  * 사용자가 버튼을 눌러야만 다음 화면으로 이동
- **로고 버튼 클릭 영역 확대**: 💙 하트 뿐만 아니라 "소곤 SOGON" 텍스트도 클릭 가능
  * 모든 페이지(채팅, 대시보드, 프리미엄, 프로필)의 로고 버튼 개선
  * pointer-events-none으로 내부 요소가 클릭을 방해하지 않도록 수정
